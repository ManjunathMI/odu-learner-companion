...
export async function GET(req: NextRequest, { params }: Context) {
  ...
  if (error) return Response.json({ error: 'Failed to load progress' }, { status: 500 });
  return Response.json({ completedItemIds: (data || []).map((row) => row.item_key) });
}
...
export async function POST(req: NextRequest, { params }: Context) {
  ...
  const { error } = await adminClient.from('progress').upsert({ user_id: user.id, path_id: pathId, item_key: body.itemId, done: body.done, updated_at: new Date().toISOString() }, { onConflict: 'user_id,item_key' });
