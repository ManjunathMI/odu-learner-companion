    ...
    <main className="discovery-page">
      <section className="discovery-hero" aria-labelledby="discovery-heading">
        <div className="hero-copy">
          ...
          <div className="hero-actions">
            ...
            <Link className="button-secondary" href="/auth">Start learning</Link>
          </div>
        </div>
      ...
      <section className="path-section" id="learning-paths" aria-labelledby="paths-heading">
        ...
        {error ? <div className="state-block error-state"><h3>Learning paths are unavailable</h3><p>{error}</p><button className="button-secondary" type="button" onClick={() => window.location.reload()}>Try again</button></div> : <div className="path-grid">
          {filteredPaths.map((path) => <Link className="path-card" href={`/paths/${path.id}`} key={path.id}>
            <div className="card-top"><div><span className="path-label">Public Learning Path</span><h3>{path.title}</h3></div><span className="card-link">View path</span></div>
            <p>{path.description || 'A structured learning path shared by the community.'}</p>
            <div className="card-footer"><span>{path.memberCount} member{path.memberCount === 1 ? '' : 's'}</span><small>{path.tags?.length ? path.tags.join(' · ') : 'Community path'}</small><span className="card-action">Open Learning Space</span></div>
          </Link>)}
      ...
      <style jsx>{`
        ...
        .path-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:1rem}.path-card{display:flex;flex-direction:column;min-height:235px;padding:1.35rem;border:1px solid var(--text-tertiary);border-radius:var(--radius-md);background:var(--bg-secondary);box-shadow:var(--shadow-sm);transition:box-shadow .15s ease,border-color .15s ease,transform .15s ease}.path-card:hover{border-color:var(--accent-primary);box-shadow:var(--shadow-md);transform:translateY(-1px)}.card-top{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem}.path-label{color:var(--accent-primary);font-size:.72rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase}.card-top h3{margin:.35rem 0 0;font-size:1.25rem}.card-link{color:var(--accent-primary);font-size:.8rem;font-weight:800;white-space:nowrap}.path-card>p{flex:1;margin:1rem 0;color:var(--text-secondary);font-size:.92rem}.card-footer{display:grid;grid-template-columns:auto 1fr;align-items:center;gap:.45rem .75rem;margin-top:auto;padding-top:.85rem;border-top:1px solid var(--border-color);color:var(--text-secondary)}.card-footer small{color:var(--text-tertiary);text-align:right}.card-action{grid-column:1/-1;color:var(--accent-primary);font-size:.85rem;font-weight:800}
