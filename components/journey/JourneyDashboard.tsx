...
export default function JourneyDashboard() {
  ...
  return <main className="journey-page">
    ...
    {paths.length > 0 ? <section aria-labelledby="active-spaces-heading"><div className="section-heading"><div><p className="section-kicker">Your current learning</p><h2 id="active-spaces-heading">Active Learning Spaces</h2></div></div><div className="journey-grid">{paths.map((path) => { const lp = path.learning_paths; if (!lp) return null; return <LearningPathCard key={path.path_id} href={`/paths/${lp.id}`} contextLabel={path.role === 'admin' ? 'Space Creator' : path.role === 'moderator' ? 'Moderator' : 'Learner'} title={lp.title} description={lp.description || 'Continue your structured learning path with the community.'} topMeta={`${path.completed} done`} progress={<div className="progress-track" aria-label={`${path.completed} lessons completed`}><span style={{ width: `${path.total ? Math.min(100, Math.round((path.completed / path.total) * 100)) : 0}%` }} /></div>} secondaryAction={path.role === 'admin' ? <Link className="text-link" href={`/paths/${lp.id}/settings`}>Manage</Link> : undefined} />; })}</div></section> : <section className="empty-journey"><p className="section-kicker">A fresh start</p><h2>Choose something worth learning.</h2><p>Your Journey will become the place to see progress, notes, and your next step once you join a Learning Space.</p><div className="empty-journey-actions"><Link className="button-primary" href="/explore">Explore Learning Paths</Link><Link className="button-secondary" href="/paths">Create a Learning Path</Link></div></section>}
    ...
    <style jsx>{`
      ...
      .empty-journey-actions{display:flex;flex-wrap:wrap;gap:.75rem}
      @media(max-width:700px){.journey-page{padding:2.5rem 1rem 4rem}.journey-header{align-items:flex-start;flex-direction:column}.journey-header h1{font-size:3rem}.journey-overview{grid-template-columns:1fr}.pending-row{align-items:flex-start;flex-direction:column;gap:.25rem}.empty-journey-actions{align-items:stretch;flex-direction:column}}
    `}</style>
  </main>;
}