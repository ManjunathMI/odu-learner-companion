...
export default function JourneyDashboard() {
  ...
  return <main className="journey-page">
    ...
    {paths.length > 0 ? <section aria-labelledby="active-spaces-heading"><div className="section-heading"><div><p className="section-kicker">Your current learning</p><h2 id="active-spaces-heading">Active Learning Spaces</h2></div></div><div className="journey-grid">{paths.map((path) => { const lp = path.learning_paths; if (!lp) return null; return <article className="journey-card" key={path.path_id}><div className="journey-card-top"><div><span className="role-label">{path.role === 'admin' ? 'Space Creator' : path.role === 'moderator' ? 'Moderator' : 'Learner'}</span><h3>{lp.title}</h3></div><span className="progress-number">{path.completed} done</span></div><p>{lp.description || 'Continue your structured learning path with the community.'}</p><div className="progress-track" aria-label={`${path.completed} lessons completed`}><span style={{ width: `${path.total ? Math.min(100, Math.round((path.completed / path.total) * 100)) : 0}%` }} /></div><div className="journey-card-footer"><Link className="button-primary" href={`/paths/${lp.id}`}>Open Learning Space</Link>{path.role === 'admin' && <Link className="text-link" href={`/paths/${lp.id}/settings`}>Manage</Link>}</div></article>; })}</div></section> : <section className="empty-journey"><p className="section-kicker">A fresh start</p><h2>Choose something worth learning.</h2><p>Your Journey will become the place to see progress, notes, and your next step once you join a Learning Space.</p><Link className="button-primary" href="/explore">Explore public paths</Link></section>}
